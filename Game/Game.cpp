...
int main() {
    ...
    // bool isLocked = true;
    //
    // GraphicsDevice.GetIO().Mouse.Lock();
    //
    ...
    //     const auto& Keyboard = GraphicsDevice.GetIO().Keyboard;
    //     auto& Mouse = GraphicsDevice.GetIO().Mouse;
    ...
    logger.SetNameColor(Bold(Debug::ConsoleColor::FG_MAGENTA));
    logger.SetIncludeFunc(false);
    logger.SetLocPad(0);

    int x = 10;
    logger.Trace({"x has size of {}"}, sizeof(x));
    logger.Debug({"x has type {}"}, stdu::nameof<decltype(x)>());
    logger.Info({"x is {}"}, x);
    logger.Warn({"x is not {}"}, ~x);
    logger.Error({"x in hex is {:x}"}, x);
    logger.Critical({"x in HEX is {:X}"}, x);
    logger.AssertEq(x, 11);
