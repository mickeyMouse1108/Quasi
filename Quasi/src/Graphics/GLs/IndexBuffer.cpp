...
namespace Quasi::Graphics {
    IndexBuffer::IndexBuffer(u32 size) : bufferSize(size) {
        Create();
        GL_CALL(glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(u32) * size, nullptr, GL_DYNAMIC_DRAW));
    }

    void IndexBuffer::SetData(Span<const u32> data, u32 dOffset) {
        Bind();
        GL_CALL(glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, dOffset, data.size_bytes(), data.data()));
    }
